...
- `drain_type` (*can be `logs` (similar to empty), `metrics` or `all`*, useable if operator didn't disallow metrics with `disable_drain_type` in config ): Allow metrics or both logs and metrics to be send in logservice.
(**Warning** Metrics should be use when you have not prometheus, a lot of dashboards are already available on it)
- `use_tls` (*boolean*, useable if operator not set `prefer_tls` in config ): Set to `true` for making cloud foundry send logs encrypted to logservice
